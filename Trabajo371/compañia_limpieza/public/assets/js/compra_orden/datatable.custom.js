function format ( d ) {
    if((d[1] == '') && (d[2] == '')  ){
        return 'No Esta Finalizado'
    }
    return 'Estado: '+d[1]+'<br>'+
            'Observacion:' + d[2];
};
...
$(document).ready(function() {
    ...
    $('.SendSelect').on('click', function() {
        const data =  $('.selected').map(function() {
            return 'ids[]=' +$(this).data('id'); 
            }).get();
        let url = $(this).data('url') + "?"+ data.join('&');
    ...
    $('.SendSelect').toggle(false);
    $('#file_input').toggle(false);
    $('#topdf2 tbody').on('click', '.cdetail', function(){
        location.href = $(this).parent('tr').data('url');
    });
    $('#topdf2 tbody').on('click', 'tr', function(){
        $(this).toggleClass('selected');
        let hayRow = $('.selected').map(function() {
        return $(this).data('id'); 
        }).get().length > 0;
            $('.SendSelect').toggle(hayRow);
            $('#file_input').toggle(hayRow);
        });
    $('#topdf2').dataTable( {
        "language": {
            "url": "//cdn.datatables.net/plug-ins/1.10.16/i18n/Spanish.json"
