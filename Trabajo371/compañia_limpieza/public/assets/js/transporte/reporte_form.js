...
{
	$("#proveedor").select2();
	$("#transporte_reporte_form").submit(function( event ) 
	{
		event.preventDefault();
	  	...
	  	$.ajax({
	          ...
	          success: function(resp)
	          {
          			...
          			$("#transporte_reporte_form").find("strong[id]").text('');
                  	// Estilo de bootstrap para marcar que estan correctos los datos
                  	$("#transporte_reporte_form").find('input:text, input:password, input:file, input[name="email"],select, textarea').removeClass('is-invalid is-valid');
	          ...
	          error: function(xhr, ajaxOptions, thrownError) 
	          {
	          		...
	          		if (xhr.status == 422) 
	          		{
	                  	...
	                  	$("#transporte_reporte_form").find("strong[id]").text('');
	                  	// Estilo de bootstrap para marcar que estan correctos los datos
	                  	$("#transporte_reporte_form").find('input:text, input:password, input:file, input[name="email"],select, textarea').removeClass('is-invalid').addClass('is-valid');
