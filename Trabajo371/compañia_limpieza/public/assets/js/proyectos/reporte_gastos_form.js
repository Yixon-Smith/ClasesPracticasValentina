$(document).ready(function ()
{
	...
	$("#reporte_gastos_form").submit(function( event ) 
	{
		event.preventDefault();
	  	...
	  	$.ajax({
	          ...
	          success: function(resp)
	          {
          			...
          			$("#reporte_gastos_form").find("strong[id]").text('');
                  	// Estilo de bootstrap para marcar que estan correctos los datos
                  	$("#reporte_gastos_form").find('input:text, input:password, input:file, input[name="email"],select, textarea').removeClass('is-invalid is-valid');
	              	...
	              	{
	              		swal("Alerta",resp.message, "warning");
	              	}
	          },
	          error: function(xhr, ajaxOptions, thrownError) 
	          {
	          		...
	          		if (xhr.status == 422) 
	          		{
	                  	...
	                  	$("#reporte_gastos_form").find("strong[id]").text('');
	                  	// Estilo de bootstrap para marcar que estan correctos los datos
	                  	$("#reporte_gastos_form").find('input:text, input:password, input:file, input[name="email"],select, textarea').removeClass('is-invalid').addClass('is-valid');
